...
- **Client-side Image Compression & Storage**: Attached review images are compressed client-side using `HTML5 Canvas` (max 1600x1600 pixels, `0.85` JPEG quality) and uploaded to Firebase Storage under `reviews/{restaurantId}/{fileName}`. Only the public download URL is stored in the Firebase Realtime Database. This prevents database bloat, maintains fast real-time synchronization, and delivers high-resolution, sharp images.
